<template>
    ...
    <div class="fixed inset-0 flex items-center justify-center z-50  ">
      <div class="w-[30%] shadow-lg bg-white ">
        ...
        <div class="h-auto min-h-10 w-full flex items-center justify-between  border-b">
          <span class="text-lg font-semibold capitalize px-2">{{ editableCategory.name }}</span>
          <button
        ...
        <div class="flex flex-row p-2 items-center justify-center border-t-2 ">
            <span @click="saveCategroyData" class="w-auto  px-3  text-white bg-green-500 hover:bg-green-600 text-center text-lg font-sans font-semibold cursor-pointer">save</span>
  ...
  export default {
    ...
    data() {
      return {
        editableCategory: { ...this.category }, // Local copy to avoid directly modifying the prop
      };
    },
    methods: {
      ...mapActions(["SaveCategory"]),
      close() {
        this.$emit("closeModal", this.editableCategory); // Emit updated category on close
      },
      saveCategroyData(){
        this.SaveCategory(this.editableCategory)
        this.close();
      }
    },
  };
